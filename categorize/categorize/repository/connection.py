...
여기엔 SQL이 없다 — 테이블을 읽고 쓰는 SQL은 전부 `repository/analysis.py`(DbStore, 파이프라인 데이터) ·
`repository/photos.py`(대상 조회) · `repository/jobs.py`(잡 실패 표시)에 있다. 두 모듈이 한 커넥션을 나눠 쓰므로 트랜잭션 경계는 호출자가 잡는다.
"""
