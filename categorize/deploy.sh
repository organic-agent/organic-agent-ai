#!/usr/bin/env bash
# categorize/ 를 이미지로 만들어 ECR에 올리고, Lambda가 그 이미지를 집게 한다.
#
# 사용법: categorize/deploy.sh        — 어느 디렉토리에서 실행해도 된다
#
# 이 함수는 앱과 배포 경로가 다르다. cd.yml 은 categorize/ 를 건드리지 않으므로 main 에
...
REPO_NAME="wes-categorize"
FUNCTION_NAME="wes-categorize"
TAG="latest"

# 스크립트가 있는 곳이 빌드 컨텍스트다. Dockerfile 이 categorize/ 안에 있고
# COPY categorize/ 가 이 디렉토리를 기준으로 잡힌다.
...
echo
echo "  * Bedrock 을 부른다. 실행 역할에 bedrock:InvokeModel(크로스 리전 프로필 포함), NAT 없는 서브넷이므로"
echo "    bedrock-runtime 인터페이스 VPC 엔드포인트가 있어야 한다."
