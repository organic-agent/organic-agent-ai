#!/usr/bin/env bash
# preference/ 를 이미지로 만들어 ECR에 올리고, Lambda가 그 이미지를 집게 한다.
#
# 사용법: preference/deploy.sh        — 어느 디렉토리에서 실행해도 된다
#
# 이 함수는 앱과 배포 경로가 다르다. cd.yml 은 preference/ 를 건드리지 않으므로 main 에
...
REPO_NAME="wes-preference"
FUNCTION_NAME="wes-preference"
TAG="latest"

# 스크립트가 있는 곳이 빌드 컨텍스트다. Dockerfile 이 preference/ 안에 있고
# COPY preference/ 가 이 디렉토리를 기준으로 잡힌다.
...
METADATA=$(mktemp -t preference-build.XXXXXX)
