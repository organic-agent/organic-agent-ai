...
from photoselect.v1.axes import AXES
from photoselect.v1.config import ScoreKnobs, Settings
from photoselect.v1.draft import job, rerank, scoring
from photoselect.v1.store import Evidence, LocalStore, PhotoAnalysis
...
def test_explain_selection_slots_match_plain_selection(world):
    """explain_selection 은 select_with_coverage 와 같은 사진을 같은 순서로 고르고, slot 이 정의와 맞는다."""
    st, settings, rows = world
    ids, E = st.read_embeddings("g")
    score = np.array([r.technical_pct + r.aesthetic_pct for r in rows])
    k = settings.score
    args = (score, E, [r.scene for r in rows], [r.cluster_id for r in rows],
            k.top_k, k.lambda_mmr, k.min_per_scene, k.scene_cap_ratio)
    picks = rerank.explain_selection(*args)
    assert [p.index for p in picks] == rerank.select_with_coverage(*args)
    slots = {p.slot for p in picks}
    assert slots <= {"quota", "diversity", "fill"} and "quota" in slots
    for p in picks:
        assert p.scene == rows[p.index].scene
        if p.slot == "quota":
            assert 1 <= p.scene_rank <= p.scene_quota
        if p.slot == "diversity":
            assert p.scene_rank > p.scene_quota
