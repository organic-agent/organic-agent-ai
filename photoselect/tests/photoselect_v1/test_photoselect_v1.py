"""photoselect_v1 단위 테스트 — 합성 데이터로 임베딩 그룹·naming(가짜 LLM)·LocalStore 왕복을 돈다. 모델 없음. (구 v3 테스트)"""
...
from photoselect_v1.compare import verdict as compare
from photoselect_v1.store import ConceptAssignment, FolderSetMissing, LocalStore, PhotoAnalysis
...
def _pa(pid, tech=50.0, aes=50.0, sharp=100.0, cluster=-1, cluster_rank=0, subjects="couple"):
    return PhotoAnalysis(photo_id=pid, subjects=subjects, technical_pct=tech, aesthetic_pct=aes,
                         ...
                         cluster_rank=cluster_rank, model_version=MODEL_VERSION)
