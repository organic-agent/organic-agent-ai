"""v2 DbStore 왕복 테스트 (v1 사본 + concept_id) — wes Flyway(V29)가 적용된 Postgres가 필요하다.
...
def _rows(photo_ids):
    from photoselect.v2.store import PhotoAnalysis
...
def test_write_analysis_keeps_embedder_vectors(conn, seed):
    from photoselect.v2.config import Settings
    from photoselect.v2.store import DbStore
...
def test_evidence_and_recommendations_are_per_selection(conn, seed):
    from photoselect.v2.config import Settings
    from photoselect.v2.store import DbStore, Recommendation
...
def test_concept_id_round_trips_via_sub_scores(conn, seed):
    """DB 에 concept_id 컬럼이 없어 sub_scores.concept_id 로 오간다 — wes 컬럼 추가 전까지의 계약."""
    from photoselect.v2.config import Settings
    from photoselect.v2.store import DbStore
    st = DbStore(Settings.from_env(), connection=conn)
    rows = _rows(seed["photos"])
    for i, r in enumerate(rows):
        r.concept_id = 7 + i
    st.write_analysis(seed["gallery"], rows, ([], None))
    back = {r.photo_id: r for r in st.read_analysis(seed["gallery"])}
    assert [back[p].concept_id for p in seed["photos"]] == [7, 8, 9]
    assert all("concept_id" not in back[p].sub_scores for p in seed["photos"])
