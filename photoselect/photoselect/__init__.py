"""photoselect — AI 클러스터링 폴더화 배치. embedder 와 같은 평탄 구조(#31).
    ...
    config(설정·손잡이) · store(DB/로컬 저장소) · gallery(미리보기 로드) · llm(Bedrock) · subjects(zero-shot) ·
    jobs(잡 전이) · db · storage.  진입점: worker(잡 폴링) · __main__(CLI).

wes 잡 mode 는 그대로다: FULL = score → categorize, NAMING = categorize (worker 가 매핑).
실제 폴더(concept_folders/detail_folders)는 wes가 ai_concept_assignments 를 읽어 만든다(POST /concept-folders/ai).
폴더별 추천과 비교샷은 wes로 이관됐다(#25) — 경계는 "갤러리 전수에 torch 모델 추론이 필요한가"다.
DB 계약(V45 스키마, model_version "photoselect-v3-a-0.1", 결과 payload의 pipeline="v3")은 그대로다.
"""
