...
def select_with_coverage(score: np.ndarray, emb: np.ndarray, scenes: list[str], cluster_ids: list[int],
    ...
    """초안 K장. exclude는 후보에서 뺄 인덱스(이미 담은 사진 — plan.md §3-B 후보 집합)."""
    ...
    if len(chosen) < k:
        rest = [i for i in np.argsort(-score) if i in set(avail) and i not in set(chosen)
                and cluster_ids[i] not in used_clusters]
        chosen.extend(int(i) for i in rest[: k - len(chosen)])
    return chosen[:k]
