"""갤러리 하나를 전수 분석한다. 진입점(`__main__ analyze` / EC2 루프)이 이 함수를 부른다.
    ...
    임베딩 (로컬 CLIP / DB는 임베더 DINOv3) →  근접 중복 클러스터 (A-6)
...
from photoselect.v1.config import MODEL_VERSION, Settings
from photoselect.v1.gallery import PhotoRef
from photoselect.v1.store import PhotoAnalysis, Store
...
def run(store: Store, gallery: str, refs: list[PhotoRef], settings: Settings,
    ...
    # ── 모델 로딩 (여기서 한 번) ─────────────────────────────────────────
    t0 = time.monotonic()
    from photoselect.v1.runners import ArniqaRunner, FacesRunner, LaionRunner
    faces, laion, arniqa = FacesRunner(), LaionRunner(), ArniqaRunner()
    tagger = None
    if use_vlm:
        from photoselect.v1.analyze.vlm import OllamaTagger
    ...
    for i, ref in enumerate(todo, 1):
        try:
            ...
            # 저장소에 이미 벡터가 있으면(DB 모드: 임베더의 DINOv3) 그것을 쓴다. CLIP 벡터는
            # 위 미학 점수에만 쓰였다. 로컬 모드는 재분석 대상이 prev_emb_map에 없으므로 그대로 CLIP.
            embs.setdefault(ref.photo_id, emb)
            result.processed += 1
