...
def pipeline_of(settings):
    """v1/v2 파사드 모듈. settings 는 루트 Settings 든 버전 Settings 든 `pipeline` 만 본다."""
...
def _one_analysis(settings, use_vlm: bool) -> bool:
    st = pipeline_of(settings).store.DbStore(settings)
    try:
        job_id = jobs.claim_next(st.conn, jobs.ANALYSIS)
        if job_id is None:
            return False
        gallery_id = jobs.analysis_job_info(st.conn, job_id)
        log.info("[analysis] job=%s gallery=%s 시작", job_id, gallery_id)
        try:
...
def loop(settings, *, use_vlm: bool = True, llm=None, poll_seconds: float = 2.0, once: bool = False) -> None:
    ...
    while True:
        did = _one_analysis(settings, use_vlm) or _one_selection(settings, llm)
        if did:
            continue
        if once:
            return
