...
대괄호는 4단계 — `subjects_trusted` 이고 담은 사진이 충분할 때만 붙는다. BT·λ 곡선은 없다:
선호와 균형은 셀 수 있는 집계값이고, 그 값이 곧 근거 문장이다.
"""
...
from photoselect.v2.config import V2Knobs
...
def prior(technical_pct: np.ndarray, aesthetic_pct: np.ndarray, k: V2Knobs) -> np.ndarray:
...
def combine(prior_raw: np.ndarray, types: list[str] | None, stats: dict | None, k: V2Knobs
