"""v2 근거 — 사실(수치·셈) + 사진 → LLM 이 "셀렉터의 말"로. 실패하면 결정적 템플릿.

재료 두 층:
  1. 검증된 수치·셈 (docs/plan-v2-slim.md §3.3) — 숫자·순위·장수는 여기서만 나온다
  ...
  2. 사진 자체 (`LlmKnobs.reasons_vision`) — 이 컷과 형제 컷 몇 장을 이미지로 같이 준다. 조명·구도·시선·
     표정·색처럼 "눈에 보이는 것"은 모델이 직접 보고 말한다. 형제와 무엇이 다른지도 보고 비교한다.

원칙: 숫자는 재료에 있는 것만, 눈에 보이는 것은 사진에서 본 것만. 톤은 부부 옆에서 결정을 밀어 주는
셀렉터 — 칭찬을 아끼지 않고 설득한다. 사진을 못 받았으면(경로 없음·vision 꺼짐) 1층 재료만으로 짧게 간다.
"""
...
from photoselect.v2.llm.client import LlmClient, Part, jpeg_bytes
...
#: 사진 없이(텍스트 재료만) 만든 문장의 상한 — 옛 동작과 같다.
