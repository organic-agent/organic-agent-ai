...
FULL 잡은 이 뒤에 naming(photoselect.v3.naming)까지 이어 돈다 — 그 배선은 worker가 한다.
재개: 같은 V3_MODEL_VERSION 이고 임베딩·CLIP이 모두 저장된 사진은 건너뛴다.
"""
...
from photoselect.v3 import cluster, concept
from photoselect.v3.config import V3_MODEL_VERSION, Settings
from photoselect.v3.gallery import PhotoRef
from photoselect.v3.store import PhotoAnalysis, Store
...
def run(store: Store, gallery: str, refs: list[PhotoRef], settings: Settings, force: bool = False,
    ...
    previous = {} if force else {
        r.photo_id: r for r in store.read_analysis(gallery) if r.model_version == V3_MODEL_VERSION}
    ...
    t0 = time.monotonic()
    from photoselect.v3 import classical
    from photoselect.v3.runners import ArniqaRunner, LaionRunner
    laion, arniqa = LaionRunner(), ArniqaRunner()
    tagger = None
    if k.subjects_zero_shot:
        from photoselect.v3.subjects import SubjectsTagger
    ...
    for i, ref in enumerate(todo, 1):
        try:
            ...
            rows[ref.photo_id] = PhotoAnalysis(
                ...
                sub_scores=sub, model_version=V3_MODEL_VERSION,
            )
