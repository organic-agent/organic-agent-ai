...
ai-folder-structure.md의 ②~④ 구현. 층마다 잘하는 도구:
    ...
    ② 이름   크기순으로 사진 커버리지 목표(naming_coverage, 상한 naming_max_groups)까지 고른
             ...
             · 부모 = 촬영 종류별 닫힌 목록 (JSON 스키마 enum으로 강제, 목록 밖이면 '기타'+proposed)
             · 컨셉 = 열린 이름
...
from photoselect.v3.analyze import concat_space
from photoselect.v3.config import PARENT_PROMPTS, PARENTS, Settings
from photoselect.v3.llm.client import LlmClient, jpeg_bytes
from photoselect.v3.store import ConceptAssignment, Store
...
def run(store: Store, gallery: str, settings: Settings, llm: LlmClient | None,
    ...
    k = settings.v3
    shoot = store.shoot_type(gallery)
    parents = PARENTS.get(shoot, PARENTS["REHEARSAL"])
    ...
    if verifier is None:
        from photoselect.v3.runners import LaionRunner
    ...
    return {
        "gallery": gallery, "pipeline": "v3", "mode": "naming", "shootType": shoot,
