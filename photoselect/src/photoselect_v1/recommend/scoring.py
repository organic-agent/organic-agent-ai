...
from photoselect_v1.config import Knobs
...
def prior(technical_pct: np.ndarray, aesthetic_pct: np.ndarray, k: Knobs) -> np.ndarray:
...
def type_stats(types: list[str], selected_mask: np.ndarray) -> dict[str, dict[str, float]]:
    ...
    for t in sorted(set(types)):
        ...
        out[t] = {"pool_share": pool, "deficit": max(0.0, pool - sel),
                  "select_lift": p_sel_given_t - p_sel, "pool_count": float(len(idx)),
                  "sel_count": float(selected_mask[idx].sum())}
    return out


def combine(prior_raw: np.ndarray, types: list[str] | None, stats: dict | None, k: Knobs
