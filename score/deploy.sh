#!/usr/bin/env bash
# score/ 를 이미지로 만들어 ECR에 올리고, Lambda가 그 이미지를 집게 한다.
#
# 사용법: score/deploy.sh        — 어느 디렉토리에서 실행해도 된다
#
# 이 함수는 앱과 배포 경로가 다르다. cd.yml 은 score/ 를 건드리지 않으므로 main 에
...
# 스크립트가 있는 곳이 빌드 컨텍스트다. Dockerfile 이 score/ 안에 있고
# COPY score/ 가 이 디렉토리를 기준으로 잡힌다.
...
METADATA=$(mktemp -t score-build.XXXXXX)
