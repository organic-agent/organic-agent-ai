...
실행 단위가 아닌 것(계획·아키텍처·개선 기록·실측·학습 노트)은 전부 `docs/` 아래다 — 네 갈래로 나뉘어 있고 기준은 `docs/README.md` 다. `docs/` 는 `.gitignore` 로
저장소에 올리지 않는 로컬 문서다(#45). 최상위에 새 디렉토리를 만드는 것은 곧 새 배포 단위를 만드는 것이다.
...
- 문서만 바꾸는 소규모 변경(`CLAUDE.md` · `.claude/rules/` · 모듈 README)은 이슈·PR 없이 `main` 직접 커밋을
  허용한다 (형식: `[<모듈>] docs: ...` 또는 `docs: ...`). `docs/` 는 저장소 밖이라 해당 없다. 코드 변경은 반드시 PR을 거친다.
