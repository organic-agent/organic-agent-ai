...
echo "  * 페이로드는 {galleryId, photoIds} 와 관리자 {jobId} 둘뿐이다(#100). 타임아웃 앞에서 멈춰도"
echo "    자기 재호출을 하지 않는다 — 남은 장은 wes 스위퍼가 dispatched_at 을 되돌려 다시 배정한다."
echo "    lambda:InvokeFunction(자기 함수)은 더 필요 없다 — 인프라에 제거를 요청할 것."
